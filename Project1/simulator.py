...
def initWorld(canvas):
    ...
    for a in agents:
        circles.append(canvas.create_oval(0, 0, a.radius, a.radius,
                                          fill=colors[a.gid % 4]))  # color the disc of an agenr based on its group id
...
def drawWorld():
    ...
    for i in range(len(agents)):
        agent = agents[i]
        if not agent.atGoal:
            canvas.coords(circles[i], world_scale * (agent.pos[0] - agent.radius - world_xmin),
                          world_scale * (agent.pos[1] - agent.radius - world_ymin),
                          world_scale * (agent.pos[0] + agent.radius - world_xmin),
                          world_scale * (agent.pos[1] + agent.radius - world_ymin))
            canvas.coords(velLines[i], world_scale * (agent.pos[0] - world_xmin),
                          world_scale * (agent.pos[1] - world_ymin),
                          world_scale * (agent.pos[0] + agent.radius * agent.vel[0] - world_xmin),
                          world_scale * (agent.pos[1] + agent.radius * agent.vel[1] - world_ymin))
            canvas.coords(gvLines[i], world_scale * (agent.pos[0] - world_xmin),
                          world_scale * (agent.pos[1] - world_ymin),
                          world_scale * (agent.pos[0] + agent.radius * agent.gvel[0] - world_xmin),
                          world_scale * (agent.pos[1] + agent.radius * agent.gvel[1] - world_ymin))
            if drawVels:
    ...
    if reachedGoals or ittr > maxIttr or QUIT:  # Simulation Loop
        ...
        if not paused:
            ...
            for agent in agents:
                if not agent.atGoal:
                    trajectories.append(
                        [agent.id, agent.gid, agent.pos[0], agent.pos[1], agent.vel[0], agent.vel[1], agent.radius,
                         globalTime])
